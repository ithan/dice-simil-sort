...
const normalizationCache: Map<string, string> = new Map();
...
function sortArrayBySimilarity<T = string>(
  ...
  const {
    usePrefix = false,
    useFunction = (item: any) => String(item)
  } = options;
...
export { normalizationCache, normalizeString, diceCoefficient, sortArrayBySimilarity };